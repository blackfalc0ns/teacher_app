...
class _TeacherTaskCreatePageState extends State<TeacherTaskCreatePage> {
      ...
      body: ListView(
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.tips_and_updates_outlined, color: AppColors.white, size: 20),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(
                    'أنشئ مهمة فردية لطالب محدد، ثم قسّمها إلى مراحل واضحة مع إثبات مناسب لكل مرحلة ومكافأة محفزة كهدية أو تقييم مادي.',
          ...
          TeacherTaskBasicsCard(
            ...
            onStudentChanged: (value) => setState(() => _selectedStudentId = value),
            onRewardTypeChanged: (value) => setState(() => _rewardType = value ?? TeacherTaskRewardType.moral),
            dueDate: _dueDate,
          ...
          TeacherTaskStagesCard(
            ...
            onStageChanged: (index, stage) => setState(() => _stages[index] = stage),
          ),
  ...
  void _submit() {
    ...
    final validStages = _stages.where((stage) => stage.title.trim().isNotEmpty).toList();
    ...
    if (_titleController.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('أكمل البيانات الأساسية والمراحل قبل الإسناد.')),
      );
      return;
    }
